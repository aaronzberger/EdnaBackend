...
from optimize import HouseDistances
import subprocess
...
class Optimizer():
    ...
    def __init__(self, cluster: list[Segment], num_lists: int, starting_location: Point):
        ...
        distance_matrix = {'profile': 'person'}
        distance_matrix['travelTimes'] = []
        ...
        for i, house in enumerate(self.points):
            ...
            job['pickups'] = [{
                'places': [
                    {'location': {'index': i},
                     'duration': MINS_PER_HOUSE * 60}],
                'demand': [1]
            }]
            problem['plan']['jobs'].append(job)

        # Create the fleet
        problem['fleet'] = {}
        problem['fleet']['vehicles'] = []
        person_dict = {
            'typeId': 'person',
            'vehicleIds': ['walker_{}'.format(i) for i in range(num_lists)],
            'profile': {'matrix': 'person'},
            'costs': {
                'fixed': 0,
                'distance': 0,
                'time': 1
            ...
            'shifts': [{
                'start': {'earliest': '2022-08-01T05:00:00Z',
                            'location': {'index': self.start_idx}},
                'end': {'latest': '2022-08-01T08:00:00Z',
                        'location': {'index': self.start_idx}}
            }],
            'capacity': [100],
            'limits': {
                'shiftTime': self.MAX_TIME_PER_LIST,
                'maxDistance': 3200,
                # 'tourSize': 100
            }
        }
        problem['fleet']['vehicles'].append(person_dict)
        problem['fleet']['profiles'] = [{'name': 'person'}]
    ...
    def optimize(self):
        ...
        # geojson_solution_path = os.path.join(BASE_DIR, 'optimize', 'solution.geojson')
