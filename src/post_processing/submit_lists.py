...
arg_parser = argparse.ArgumentParser(
    description="Retrieve new house images from Street View"
)
arg_parser.add_argument(
    "-d", "--directory", type=str, help="Directory of lists", required=True
)
arg_parser.add_argument(
    "-k", "--key", type=str, help="Google API key", default=os.environ["GOOGLE_API_KEY"]
)
arg_parser.add_argument(
    "-a",
    "--approve",
    action="store_true",
    help="Automatically approve downloading images",
)
...
def process_list(filename: str, save_dir: str):
    ...
    for block in walk_list["blocks"]:
        for house in block["houses"]:
            addresses.append(
                f"{house['display_address']}, {house['city']}, {house['state']}, {house['zip']}".replace(
                    "None", ""
                )
            )
    ...
    for uuid, address in tqdm.tqdm(
        ...
        if r.status_code != 200:
            ...
            if os.path.isfile("{}/{}.jpg".format(save_dir, uuid)):
                ...
                os.rename(
                    "{}/{}.jpg".format(save_dir, uuid),
                    "{}/{}/{}.jpg".format(save_dir, "upload", uuid),
                )
                num_to_upload += 1
                continue
    ...
    print(
        f"Of {len(uuids)} images, {num_found} already exist, {num_failed} failed, {num_to_upload} need uploaded, and {num_needed} need requested"
    )
    ...
    if not args.approve:
        print(
            colored(
                "Confirm requesting {} houses from street view? (y/n)".format(
                    len(needed_images)
                ),
                "yellow",
            )
    ...
    for image in tqdm.tqdm(
        ...
        if not os.path.isfile("{}/gsv_0.jpg".format(save_dir)):
            print(
                "Error: Failed to download image for {}: {}".format(image[0], image[1])
            )
        ...
        os.rename(
            "{}/gsv_0.jpg".format(save_dir), "{}/{}.jpg".format(save_dir, image[0])
        )
    ...
    print(
        colored(
            "Retrieved and wrote {} images to {}".format(len(needed_images), save_dir),
            "green",
        )
    )
...
lists = [
    ...
    if os.path.isfile(os.path.join(args.directory, f)) and f.endswith(".json")
]
if len(lists) == 0:
...
for list_file_name in lists:
    ...
    qr.add_data(
        "https://votefalcon.s3.amazonaws.com/lists/{}.json".format(list_file_name)
    )
    qr.make(fit=True)
