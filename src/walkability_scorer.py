...
def get_house_info(block: Block, house: Point) -> HouseInfo | None:
    ...
    return next(
        filter(
            lambda x, lat=lat, lon=lon: decimalize(x["lat"]) == lat
            and decimalize(x["lon"]) == lon,
            block["addresses"].values(),
        ),
        None,
    )
...
def score(input: list[SubBlock]) -> dict[str, Any]:
    ...
    for first, second in itertools.pairwise(
        [input[0].start]
        + list(itertools.chain.from_iterable([s.houses for s in input]))
        + [input[-1].end]
    ):
    ...
    for block in input:
        ...
        for first, second in itertools.pairwise(block.houses):
            ...
            if first_house is None or second_house is None:
                print(
                    colored(
                        "Error: address {} or {} not found in block {}".format(
                            first, second, block.block["id"]
                        ),
                        color="yellow",
                    )
                )
            ...
            if first_house["side"] != second_house["side"]:
                total += (
                    first_house["distance_to_road"] + second_house["distance_to_road"]
                )
            else:
                addition = (
                    first_house["distance_to_road"]
                    + second_house["distance_to_road"]
                    - ROAD_WIDTH[block_type]
                )
                if addition < 10:
                    print(
                        colored(
                            "Warning: distance between {} and {} is less than 10 meters".format(
                                first, second
                            ),
                            color="yellow",
                        )
    ...
    return {
        ...
        "distance": total,
        "num_houses": len(
            list(itertools.chain.from_iterable([i.houses for i in input]))
        ),
    }
...
if __name__ == "__main__":
    ...
    for row in data:
        ...
        if block_id != current_block_id:
            ...
            current_sub_block = SubBlock(
                ...
                start=_blocks[block_id]["nodes"][0],
                end=_blocks[block_id]["nodes"][-1],
                extremum=None,
                houses=[],
                navigation_points=_blocks[block_id]["nodes"],
            )

        house_address: HouseInfo | None = next(
            filter(
                lambda x, address=address: x == address,
                _blocks[block_id]["addresses"].keys(),
            ),
            None,
        )
        ...
        if house is None:
            print(
                colored(
                    "Error: address {} not found in block {}".format(address, block_id),
                    color="yellow",
                )
            )
            continue
    ...
    print(
        str(scored["num_houses"])
        + ","
        + str(scored["distance"])
        + ","
        + str(scored["road_crossings"]["tertiary"])
        + ","
        + str(scored["road_crossings"]["secondary"])
        + ","
        + str(scored["road_crossings"]["residential"])
    )
