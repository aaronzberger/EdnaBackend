...
if __name__ == "__main__":
    ...
    if len(argv) == 2:
        ...
        houses_to_id: houses_file_t = json.load(open(addresses_file))
        ...
        for house in reader:
            if (
                ...
                raise ValueError(
                    "The universe file must contain either an 'Address' column or 'House Number', 'Street Name' columns"
                )
            total_houses += 1
        ...
        print(
            "Failed on {} of {} houses".format(associater.failed_houses, total_houses)
        )
    else:
