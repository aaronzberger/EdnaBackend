...
if [[ $(find . -name '*.osrm' -type f -maxdepth 1) ]]
then
    filename=$(find . -name '*.osrm' -type f -maxdepth 1 -exec basename {} .osrm \;)
    echo -e "${GREEN}Found Pre-processed OSRM data named ${filename}${NC}"
else
    echo -e "${GREEN}No OSRM file found. If you have an OSRM file already, exit and move it here. ${NC}"
    read -r -p "${WARN}Downloading northeast and pre-processing. THIS WILL TAKE many mins. Press enter to continue${NC}"
    ...
    if [[ $(find . -name '*.osrm.ebg' -type f -maxdepth 1) ]]
    then
